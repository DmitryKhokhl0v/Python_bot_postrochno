...
botStory = Bot(token=API_TOKEN) # создание экземпляра класса Bot из aiogram
dp = Dispatcher(botStory) # создание диспетчера для бота
...
async def send_welcome(message: types.Message): # функция-обработчик для команды /start
    kb = [
        [types.KeyboardButton(text="Я хочу написать свой собственный рассказ!", callback_data='ready_tw')], #Создаём на выбор две кнопки клавиатуры,
...
async def communicate(message: types.Message): #функция-обработчик для всех текстовых сообщений пользователей
    ...
    if (callback_data + '.txt') in os.listdir("data") and not now_w:
        ...
        match callback_data: # оператор сопоставления
            ...
            case "Я хочу написать свой собственный рассказ!":
